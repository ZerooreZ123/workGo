    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          <div onClick={ev => this.cancelRelease(ev)} className={styles.cancel}>
            取消
          </div>
          <div onClick={ev => this.announce(ev)} className={this.state.chooseDay ? styles.release : styles.noRelease}>
            发布
          </div>
        </div>
        <div className={styles.content}>
          ...
          <textarea className={styles.inputBlock} placeholder="公告内容" onChange={ev => this.getContent(ev)} value={this.state.announcementContent} />
        ...
        <div className={styles.dateBox}>
          <div className={styles.releaseTime}>
            公告起止日期:<span onClick={ev => this.showMask1(ev)} className={styles.buttonSlect}>
              {this.state.selectedDay}
            </span>
            <img className={styles.icon} src={down} alt="" />-<span onClick={ev => this.showMask2(ev)} className={styles.buttonSlect1}>
              {this.state.chooseDay}
            </span>
            <img className={styles.icon} src={down} alt="" />
          </div>
        </div>
        <div className={styles.footer}>
          <div onClick={ev => this.historyAnnouncement(ev)} className={styles.history}>
            历史公告
          </div>
          <div className={styles.photoBox}>
            ...
            <input
              ref="files"
              onClick={ev => this.allMask(ev)}
              onChange={ev => this.getBase64(base64 => this.upload(base64))}
              type="file"
        ...
        <div className={mask === false ? styles.hideMask : styles.showMask}>
          <div className={styles.maskHide} onClick={ev => this.hideMask1(ev)} />
          <div className={styles.maskBox}>
            <DayPicker onDayClick={ev => this.preClockInRemind(ev)} />
          </div>
        </div>
        <div className={copyMask === false ? styles.hideMask : styles.showMask}>
          <div className={styles.maskHide} onClick={ev => this.hideMask2(ev)} />
          <div className={styles.maskBox}>
