...
class EnterpriseRegistration extends Component {
  constructor() {
    ...
    this.state = {
      ...
      inputCode: "", //输入验证码
      inputPhone: window.goGoPhone, //手机号
      code: "" //返回验证码
    };
  }
  componentDidMount() {
      ...
      if (data.hasOwnProperty("companyid")) {
        if (data.roleid === "2") {
          this.props.history.replace(
            "/addAttendanceMachine/" + this.props.match.params.serialNumber + "/" + data.companyName + "/" + data.name + "/" + data.phone + "/" + data.loginName
          );
        } else {
          alert("无权限");
        }
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.invite}>
          ...
          <div className={styles.inputClass}>{inputPhone}</div>
        </div>
        ...
        <div className={styles.next}>
          <div className={inputCode && inputPhone ? styles.nextCan : styles.nextStep} onClick={ev => this.goToNextStep(ev)}>
            下一步
          </div>
        </div>
