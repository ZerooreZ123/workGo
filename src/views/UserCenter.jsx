  ...
  if (noticeState) {
    return (
      <div className={styles.noticeBoard}>
        ...
        <span onClick={ev => parent.AnnouncementDetails(ev)} className={styles.noticeText}>
          {title}
        </span>
...
const Header = ({ roleid, parent }) => {
  //个人中心头部
  if (roleid === "3") {
    return (
      <div className={styles.header}>
        <div onClick={ev => parent.scan(ev)} className={styles.scanBox}>
          <img className={styles.scanImg} src={scan} alt="" />
          <span className={styles.scanText}>后台登录</span>
        </div>
        <div onClick={ev => parent.unbindUser(ev)} className={styles.unbindButton}>
          解绑企业
        </div>
      </div>
    );
...
const Module = ({ roleid, superMan, ordinary, parent }) => {
  //个人中心不同权限展示模块
  if (roleid === "2") {
    //超级管理员
    return (
  ...
  if (prompt === 0) {
    ...
    return (
      <div className={styles.content}>
        ...
        <div className={styles.prompt}>
          <img className={styles.promptImg} src={load} alt="" />
          <span className={styles.text}>正在搜索考勤机...</span>
        </div>
    ...
    return (
      <div className={styles.content}>
        ...
        <div className={styles.prompt}>
          <img className={styles.promptImg} src={warn} alt="" />
          <span className={styles.text}>网络连接异常,蓝牙未打开</span>
        </div>
        <div onClick={ev => parent.refresh(ev)} className={styles.refreshShow}>
          刷新页面
        </div>
        <div className={styles.promptText}>搜索考勤机时请保证网络连接正常,蓝牙为开启状态哦!</div>
      </div>
      ...
      <div className={styles.content}>
        <div className={styles.clickClock}>
          ...
          <div className={styles.cardTime}>
            <div className={styles.clockDate1}>
              {h}:{m}:{s}
            </div>
          </div>
        </div>
        <div className={styles.prompt}>
          <img className={styles.promptImg} src={successMin} alt="" />
          <span className={styles.text}>打卡成功!</span>
        </div>
        <div className={styles.refreshHide}>刷新页面</div>
        <div className={styles.promptText}>搜索考勤机时请保证网络连接正常,蓝牙为开启状态哦!</div>
      </div>
    ...
    return (
      <div className={styles.content}>
        ...
        <div className={styles.prompt}>
          <img className={styles.promptImg} src={warn} alt="" />
          <span className={styles.text}>您不在考勤范围内</span>
        </div>
        <div onClick={ev => parent.refresh(ev)} className={styles.refreshShow}>
          刷新页面
        </div>
    ...
    setTimeout(() => {
      ...
      window.workgo.listenBluetoothState(result => {
        if (result.state.toString() === "poweredOn") {
          window.workgo.rangingNearBeacons(
            result => {
              if (result.success) {
              } else {
                alert(result.errMsg);
        ...
        <div>
          <div className={showUserCenter === true ? styles.moduleShow : styles.moduleHide}>
            <div className={styles.headerBox}>
              <Header roleid={roleid} parent={this} />
            ...
            <Module roleid={roleid} superMan={superMan} ordinary={ordinary} parent={this} />
          </div>
          <div className={showPunchClock === true ? styles.moduleShow : styles.moduleHide}>
            <Notice noticeState={noticeState} parent={this} title={noticeTitle} />
            <ClockPage prompt={prompt} parent={this} h={h} m={m} s={s} />
          </div>
        </div>
