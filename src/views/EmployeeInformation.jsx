...
class EmployeeInformation extends Component {
  ...
  searchPersonalInformation(i) {
    window.Person = {
      ...
      buMenId: this.state.searchDate[i].officeid,
      barState: true
    };
    this.props.history.push("/personalInformation");
  }
  goPersonalInformation(i) {
    window.Person = {
      ...
      buMenId: this.state.departmentPart[i].officeid,
      barState: true
    };
    this.props.history.push("/personalInformation");
  }
      ...
      if (this.state.mask) {
        return (
          <div className={styles.mask}>
            <div className={styles.makeHide} onClick={ev => this.hideMask(ev)} />
            <div className={styles.maskBox}>
              <div className={styles.operation}>
                <img onClick={ev => this.hideMask(ev)} className={styles.spread} src={spread} alt="" />
              </div>
              <div className={styles.determine} onClick={ev => this.determineDepartment(ev)}>
                确定
              </div>
              <div className={styles.departmentBox}>
                {section.map((item, index) => (
                  <div onClick={ev => this.clickTerm(index)} className={departmentIndex === index ? styles.selectTerm : styles.term} key={index}>
                    {item.name}
                  </div>
                ))}
              ...
              <div className={styles.footer}>
                {departmentName}
                <Direction checked={this.state.mask} parent={this} />
              </div>
            </div>
          </div>
        );
    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <div onClick={ev => this.makeSearch(ev)} className={styles.searchText}>
            搜索
          </div>
        </div>
        <div className={searchState === false ? styles.showContent : styles.hideContent}>
          <Content />
          <div className={styles.footer} onClick={ev => this.showMask(ev)}>
            {departmentName}
            <Direction checked={this.state.mask} parent={this} />
          </div>
          <Mask />
