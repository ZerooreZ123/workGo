...
class WriteInformation extends Component {
  ...
  async ShareInviteCode() {
    ...
    if (JSON.parse(result).success === "T") {
      ...
      window.sessionStorage.setItem("codeUrl", JSON.parse(result).data.codeStr);
    } else {
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.next}>
          <div onClick={ev => this.ShareInviteCode(ev)} className={inputValue && inputText ? styles.nextCan : styles.nextStep}>
            完成
          </div>
        </div>
      </div>
